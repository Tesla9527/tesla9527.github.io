...
for i in range(1, int(max_page) +1):
	...
	all_a = r.html.find('.postlist')[0].find('a[target=_blank]') #获取当前页的所有a标签元素
	for a in all_a:
		title = a.text
		if title != '':
			print("准备爬取：" + title)
			title_path = title.strip().replace('?','').replace(':','') #解决win下不能创建带'?',':'的目录的问题
			...
			for num in range(1,int(pic_max)+1):
				...
				pic_url = r.html.find('.main-image')[0].search('src="{}"')[0] #图片的最终url
				print(pic_url)
